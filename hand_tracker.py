    ...
    while cap.isOpened():
        ...
        if result.multi_hand_landmarks:
            for hand_landmarks in result.multi_hand_landmarks:
                ...
                # SHOULDER & ELBOW: Y-axis (inverted)
                shoulder_angle = map_range(lm[0].y, 0.0, 1.0, 180, 0)
                elbow_angle = map_range(lm[0].y, 0.0, 1.0, 180, 0)

                # WRIST: Z-depth using Euclidean distance from wrist to index tip
                dx = lm[8].x - lm[0].x
                dy = lm[8].y - lm[0].y
                dist = math.sqrt(dx**2 + dy**2)
                dist = max(min(dist, 0.4), 0.1)
                wrist_angle = map_range(dist, 0.1, 0.4, 0, 180)

                # CLAW: Distance between index tip and thumb tip
                thumb_tip = lm[4]
                index_tip = lm[8]
